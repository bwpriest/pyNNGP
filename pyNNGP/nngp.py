...
class NNGP(object):
    def __init__(self, t, y, eps, refType, m, cov):
        ...
        self.m = (
            m
        )  # number of reference neighbors to associate with each point t.
    ...
    def _init_s(self):
        ...
        if isinstance(self.refType, basestring):
            ...
            if self.typ == "subset":
                ...
                self.s = np.vstack(
                    [np.random.uniform(lo, hi, nRef) for lo, hi in bounds]
                ).T
    ...
    def _make_s_neighbor_sets(self):
        ...
        for i, si in enumerate(self.s):
            ...
            self.Ns.append(
                kdtree.query(
                    si.reshape(1, -1), k=min(self.m, i), return_distance=False
                )[0]
            )
